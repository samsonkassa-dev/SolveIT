...
module.exports = function (app) {
      ...
      UserAccount.findById(
        ...
        function (error, user) {
          if (
            error ||
            !user ||
            user.toJSON().role.name !== "solve-it-participants"
          ) {
            cb(null, false);
          } else {
            cb(null, true);
      ...
      UserAccount.findById(
        ...
        function (error, user) {
          if (
            error ||
            !user ||
            user.toJSON().role.name !== "solve-it-investor"
          ) {
            cb(null, false);
          } else {
            cb(null, true);
  ...
  IcogRole.registerResolver("judge", function (role, context, cb) {
    if (!context.accessToken) {
      cb(null, false);
    } else {
      UserAccount.findById(
        ...
        { include: "role" },
        function (error, user) {
          if (error || !user || user.toJSON().role.name !== "solve-it-judge") {
            cb(null, false);
          } else {
            cb(null, true);
          }
