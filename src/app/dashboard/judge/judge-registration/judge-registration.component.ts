...
export class JudgeRegistrationComponent implements OnInit {
  ...
  investmentOptions = ["Yes", "No", "Maybe"];

  genderOptions = ["Male", "Female"];

  fieldOfExpertiseOptions = ["Business", "Technical", "Other"];
  ...
  ngOnInit() {
    this.educationalBackgroundOptions = this.format(
      this.educationalBackgroundOptions
    );
    this.investmentOptions = this.format(this.investmentOptions);
    this.professionalBackgroundOptions = this.format(
      this.professionalBackgroundOptions
    );
  ...
  registerJudge(judgeForm) {
    this.authService.registerJudge({ user: judgeForm }).subscribe(
      res => {
  ...
  onItemDeselected(item) {
    this.judge.cities.splice(this.judge.cities.indexOf(item.item_id), 1);
  }
