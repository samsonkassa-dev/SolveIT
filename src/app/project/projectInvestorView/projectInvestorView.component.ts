...
export class ProjectInvestorViewComponent implements OnInit {
  ...
  constructor(public route: ActivatedRoute, public authService: AuthService, public service: ProjectService, public router: Router) {
  ...
  ngOnInit() {
    ...
    let projectViewObject = {userId: this.authService.getUserId(), projectId: id};
  ...
  rateProject() {
    this.service.rateProject(this.rating).subscribe(res => {
        if(this.findWithAttr(this.ratings, 'ratorId', this.authService.getUserId()) == -1) {
          this.ratings.push({id: res.id, ratorId: this.authService.getUserId(), review: res.review, rating: res.rating}); 
        }else {
          this.rating[this.findWithAttr(this.ratings, 'ratorId', this.authService.getUserId())].rating = res.rating;
          this.rating[this.findWithAttr(this.ratings, 'ratorId', this.authService.getUserId())].review = res.review;
        }
    }, err => {
        this.ratings[this.findWithAttr(this.ratings, 'ratorId', this.authService.getUserId())].rating = this.rating.rating;
        this.ratings[this.findWithAttr(this.ratings, 'ratorId', this.authService.getUserId())].review = this.rating.review;
    });
  }
  ...
  fetchProfile() {
    this.service.fetchInvestorProfile(this.authService.getUserId()).subscribe(res => {
